...
pwm = GPIO.PWM(BUZZER_PIN, 1)  # 初期周波数ダミー

def play_tone(frequency, duration):
    pwm.ChangeFrequency(frequency)
    pwm.start(50)           # デューティ50%
    time.sleep(duration)
    pwm.stop()
...
def gen(camera):
    ...
    while True:
        ...
        if monitoring and elapsed > monitor_duration:
            ...
            last_switch_time = current_time
            allow_duration = random.uniform(5, 10)
            print(f"★ 許可時間に切り替え（動いてもOK） duration={allow_duration:.1f}s")

        elif not monitoring and elapsed > allow_duration:
            monitoring = True
            last_switch_time = current_time
            monitor_duration = random.uniform(3, 8)
            print(f"★ 監視時間に切り替え（動いたらダメ！） duration={monitor_duration:.1f}s")
...
pwm = GPIO.PWM(BUZZER_PIN, 1)  # 仮の周波数で初期化
